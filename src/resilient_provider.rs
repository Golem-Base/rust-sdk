...
/// Wrapper error type that can hold both RPC errors and retry-specific errors.
#[derive(Debug, Error)]
pub enum RpcError<E> {
    /// Original error from the underlying operation
    #[error("{0}")]
    Original(E),
    /// Error sending request - can be retried
    #[error("{operation} failed after {attempts} attempts with error sending request: {message}")]
    ErrorSendingRequest {
        operation: String,
        attempts: u32,
        message: String,
    },
    /// No backend is currently healthy - can be retried
    #[error("{operation} failed after {elapsed_secs:.1}s with no backend healthy: {message}")]
    NoBackendHealthy {
        operation: String,
        elapsed_secs: f64,
        message: String,
...
impl<E> From<E> for RpcError<E> {
    fn from(err: E) -> Self {
        RpcError::Original(err)
    }
}
    ...
    N: Network,
{
    ...
    async fn retry<T, F, Fut, E>(
        &self,
        operation_name: &str,
        operation: F,
    ) -> std::result::Result<T, RpcError<E>>
    where
        F: Fn() -> Fut,
            ...
            match operation().await {
                ...
                Err(e) if e.to_string().contains("error sending request") => {
                    ...
                    if attempts > self.config.max_retries {
                        return Err(RpcError::ErrorSendingRequest {
    ...
    /// Makes a JSON-RPC request with retry logic for "error sending request" errors.
    /// This provides resilience against temporary network issues and RPC load balancing switches.
    pub async fn request<S: RpcSend, R: RpcRecv>(
        &self,
    ...
    pub async fn get_accounts(&self) -> Result<Vec<Address>> {
        Ok(self
            .retry("get_accounts", || self.provider.get_accounts())
            .await?)
    }
    ...
    pub async fn get_chain_id(&self) -> Result<u64> {
        Ok(self
            .retry("get_chain_id", || self.provider.get_chain_id())
            .await?)
    }

    /// Gets the syncing status with retry logic.
    pub async fn syncing(&self) -> Result<SyncStatus> {
        Ok(self.retry("syncing", || self.provider.syncing()).await?)
    }
    ...
    pub async fn get_block_by_number(
        ...
        Ok(self
            .retry("get_block_by_number", || async {
    ...
    pub async fn get_transaction_by_sender_nonce(
        ...
        Ok(self
            .retry("get_transaction_by_sender_nonce", || async {
                self.provider
